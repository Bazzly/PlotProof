...
A static "Common Questions - New to Land Surveys?" expander sits above the mode selector (visible on every wizard step), answering what a first-time, non-surveyor visitor is most likely to wonder - what a survey plan is, what coordinates are, why a result might get flagged (reusing the same `RISK_EXPLAINER` copy as the results page, so the two never disagree), and what's actually in the downloadable report. Answers are hand-written in `app_home.py`, not a live model call - fixed, reviewable, no per-question API cost, and can't hallucinate.
