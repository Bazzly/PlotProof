...
A second, separately-scoped Claude assistant (`assistant.ask_general()`), reachable from a floating button pinned bottom-right on every page (`utils/nav.py`'s `render_floating_chat()`, an `st.popover` inside a `position: fixed` container - `utils/theme.py`) rather than tucked into the results flow like the report-grounded one above. Answers general questions about Nigerian land/surveys and about PlotProof itself - explicitly instructed that it has no document or result for this specific visitor, so it can't and won't claim anything about "your land"; a question like "is my land safe" gets redirected to the real check, not a guess. Unlike `ask()`, this one keeps real conversational memory - up to the last `_MAX_CHAT_HISTORY_TURNS` (6) exchanges are replayed as actual prior turns in the Messages API call (not just summarized in text), so a follow-up "what about..." can reference the previous answer.

**Its whole body is wrapped in a try/except, and `st.popover` has a version-safe fallback to `st.expander`.** This isn't defensive-for-the-sake-of-it: an unhandled `AttributeError` inside `render_floating_chat()` took down *every single page load on the live deployment* - confirmed directly against `https://plotproof.streamlit.app/`, not a hypothetical - since `app.py`'s `st.navigation(...).run()` executes each page's whole script top to bottom, and `render_floating_chat()` runs early, before any of the actual tool renders. `requirements.txt` only pins a lower bound on Streamlit (`>=1.35.0`), so the exact deployed version isn't guaranteed identical to local dev's; `trigger = st.popover if hasattr(st, "popover") else st.expander` (verified locally by deleting `st.popover` and confirming the app still loads and the chat still works via the expander instead) covers the most likely cause, and the outer `try/except` (logs via `traceback.print_exc()`, silently doesn't render on failure) guarantees this one optional feature can never again break the entire app for every visitor, regardless of what the actual failure turns out to be.

A plain `st.page_link("app_home.py", ...)` - "Start using PlotProof - upload your document" - always renders after the Q&A history and input box, guaranteed regardless of what the model says, on top of the system prompt's own (softer, natural-language) instruction to nudge toward the real check. Same gating as the report-grounded assistant - hidden with no API key configured, and additionally hidden until the consent gate's been accepted (`render_floating_chat()` checks `st.session_state["_consent_accepted"]` itself), since it's a real per-request API cost that shouldn't be reachable before a visitor has agreed to anything. Own daily cap (`DAILY_LAND_CHAT_LIMIT`, default 10) and burst limit, separate bucket ("land_chat") from the report-grounded assistant's ("assistant_ask") so exhausting one doesn't block the other.

### Fallback when the Claude API isn't reachable

Confirmed as a real, recurring failure mode for this project specifically (not a hypothetical) - the configured Anthropic account's credit balance ran out mid-session, and every Claude-backed feature (vision extraction, both assistants) started failing with `anthropic.BadRequestError: Your credit balance is too low`. Vision extraction already degraded gracefully (falls back to free OCR - see "Vision-based extraction for photos" below); until now the two chat assistants just showed a dead-end "something went wrong" with no useful next step.

`utils/faq_content.py` factors the Common Questions page's Q&A (`pages/faq.py`) out into a plain `FAQ_ENTRIES` list - a single source of truth so a fallback answer is only ever exactly what's already reviewed and shown on that page, never improvised on the spot. `assistant.fallback_answer(question)` does a keyword-overlap match (Jaccard similarity - intersection over union of each side's meaningful words, common stopwords stripped) against every FAQ question and returns the best match's answer if it clears `_FALLBACK_MATCH_THRESHOLD` (0.5), else `None`. Deliberately conservative: dividing by the *union* rather than just the FAQ side was a real fix, not a style choice - the simpler ratio let a genuinely report-specific question like "why is my land high risk" score 0.5 against "Can I buy this land?" on the single shared word "land" alone, a false positive confirmed directly before switching the formula (extra words in the question that aren't in the FAQ question, like "high risk" there, now correctly drag the score down instead of being ignored).

Both `app_home.py`'s report-grounded handler and `utils/nav.py`'s floating-chat handler catch the API exception, try `fallback_answer()` first, and only fall through to a plain "temporarily unavailable, here's how to reach us" message if nothing matched well enough - so a visitor asking something the static FAQ actually covers still gets a real, useful answer even with the API down, clearly labeled as coming from the FAQ rather than a live response.
