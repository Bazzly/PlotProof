...
The card also shows the diagonal target's actual WGS84 coordinate, not just the bearing/distance reading - `compute_diagonal()`'s target point (`point_en`, already computed as part of finding the farthest vertex) is converted to lat/lon alongside the rest of the boundary rather than left as an internal-only projected easting/northing. Both extraction paths append it to the *same* `crs_utils.resolve_to_wgs84()` call already converting the main polygon (not a separate conversion) so it goes through the identical declared/forced/auto-detected CRS, then split back off by position; the edit-path (`resolve_recomputed_points()`) reuses the same flat-earth offset-from-origin approximation it already applies to every other vertex. `legs_info["diagonal"]["point_latlon"]` stays unset only if the converted point itself falls outside valid lat/lon range - vanishingly unlikely for a real point derived from an already-valid boundary, but checked rather than assumed.
